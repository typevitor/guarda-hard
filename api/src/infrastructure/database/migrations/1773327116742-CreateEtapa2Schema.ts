...
export class CreateEtapa2Schema1773327116742 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE IF NOT EXISTS "departamentos" ("id" varchar PRIMARY KEY NOT NULL, "empresa_id" varchar(36) NOT NULL, "nome" varchar(100) NOT NULL, "created_at" datetime NOT NULL DEFAULT (datetime('now')), "updated_at" datetime NOT NULL DEFAULT (datetime('now')))`,
    );
    await queryRunner.query(
      `CREATE TABLE IF NOT EXISTS "usuarios" ("id" varchar PRIMARY KEY NOT NULL, "empresa_id" varchar(36) NOT NULL, "departamento_id" varchar(36) NOT NULL, "nome" varchar(150) NOT NULL, "email" varchar(200) NOT NULL, "ativo" boolean NOT NULL DEFAULT (1), "created_at" datetime NOT NULL DEFAULT (datetime('now')), "updated_at" datetime NOT NULL DEFAULT (datetime('now')))`,
    );
    await queryRunner.query(
      `CREATE TABLE IF NOT EXISTS "hardwares" ("id" varchar PRIMARY KEY NOT NULL, "empresa_id" varchar(36) NOT NULL, "descricao" varchar(200) NOT NULL, "marca" varchar(100) NOT NULL, "modelo" varchar(100) NOT NULL, "codigo_patrimonio" varchar(50) NOT NULL, "funcionando" boolean NOT NULL DEFAULT (1), "descricao_problema" text, "livre" boolean NOT NULL DEFAULT (1), "version" integer NOT NULL, "created_at" datetime NOT NULL DEFAULT (datetime('now')), "updated_at" datetime NOT NULL DEFAULT (datetime('now')))`,
    );
    await queryRunner.query(
    ...
    if (
      ...
      !usuariosFKs.some(
        (fk) => fk.from === 'departamento_id' && fk.table === 'departamentos',
      )
    ) {
      await queryRunner.query(
