...
class InMemoryAuthService {
  ...
  async register(input: {
    ...
    const empresaExists = this.empresas.some((empresa) => empresa.id === input.empresaId);
    ...
    const duplicated = [...this.users.values()].some((user) => user.email === normalizedEmail);
  ...
  async login(input: { email: string; senha: string }): Promise<{ userId: string }> {
    const normalizedEmail = input.email.trim().toLowerCase();
  ...
  async listMinhasEmpresas(userId: string): Promise<Array<{ id: string; nome: string }>> {
    ...
    return this.empresas.filter((empresa) => memberships.has(empresa.id));
  }
  ...
  async selectEmpresa(input: {
    ...
    const memberships = this.membershipByUser.get(input.userId) ?? new Set<string>();
...
describe('AuthController', () => {
  ...
  it('GET /auth/empresas lists public empresas', async () => {
    const response = await request(app.getHttpServer()).get('/auth/empresas');
  ...
  it('POST /auth/login returns phase-A session cookie', async () => {
    ...
    expect(String(response.headers['set-cookie']?.[0] ?? '')).toContain('gh_session=');
  });
  ...
  it('GET /auth/me includes empresaId in phase-B', async () => {
    ...
    expect(me.body).toHaveProperty('empresaId', '11111111-1111-1111-1111-111111111111');
  });
