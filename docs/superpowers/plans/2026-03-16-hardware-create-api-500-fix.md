...
- [ ] **Step 1: Reproduce the current failure with evidence capture**
...
- [ ] **Step 2: Add temporary boundary diagnostics (UI + API status)**
...
- [ ] **Step 3: Confirm backend create endpoint behavior directly**
...
- [ ] **Step 4: Remove temporary diagnostics after confirming root cause**
...
- [ ] **Step 1: Add failing frontend contract test for create payload shape**
...
- [ ] **Step 2: Add failing form validation tests for required create fields**
...
- [ ] **Step 3: Add backend regression test for validation status stability**
...
- [ ] **Step 4: Run tests to verify initial failure state**
...
- [ ] **Step 1: Expand frontend hardware schema to match API create contract**
...
- [ ] **Step 2: Add missing inputs to Hardware form**
...
- [ ] **Step 3: Keep create API call typed against expanded payload**
...
- [ ] **Step 4: Handle API errors in server action boundary without opaque 500 UX**
...
- [ ] **Step 1: Preserve API message and status when parsing error payloads**

Ensure `toApiError` continues extracting message from backend payload and preserves HTTP status for UI-level decisions.

- [ ] **Step 2: Map common hardware-create failure statuses to clear modal messages**
...
- [ ] **Step 3: Add tests for user-visible error feedback behavior**
...
- [ ] **Step 3: Run app and API verification suites (nearest practical scope)**

Run: `pnpm --filter @guarda-hard/app test && pnpm --filter @guarda-hard/api test`
Expected: PASS with no new regressions.

- [ ] **Step 4: Manual smoke flow for hardware creation**
...
- [ ] **Step 1: Mark the corresponding PRD checklist item immediately after each completed stage**
